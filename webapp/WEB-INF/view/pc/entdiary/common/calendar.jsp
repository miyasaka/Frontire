			...
			<div class="mainDiary-side">
				<div class="listBox09">
					<div class="listBox09Title">
						<table class="ttl">
							<tr>
								<td class="ttl_name_sp"><div class="ttl_name myColStick">カレンダー</div></td>
							</tr>
						</table>
					</div>
										...
										<span class="previousMonth">
											<c:choose>
												<c:when test="${existDiary eq false}">
													<s:link href="/pc/diary/list/${f:h(userInfoDto.visitMemberId)}">&lt;</s:link>
												</c:when>
												<c:otherwise>
													<s:link href="/pc/diary/beforeMonth/${f:h(userInfoDto.visitMemberId)}">&lt;</s:link>
												</c:otherwise>
											</c:choose>
										</span>
										<span class="calMonth">
													...
													<span>
														<c:choose>
															<c:when test="${fmonth['existDiary'] eq 1}">
																<s:link href="/pc/diary/searchDay/${f:h(fmonth['detailDay']) }/${f:h(userInfoDto.visitMemberId)}">${f:h(fmonth['day']) }</s:link>
															</c:when>
															<c:otherwise>
																${f:h(fmonth['day']) }
															</c:otherwise>
...
<table>
	<caption class="clearfix">
		...
		<c:if test="${fn:substring(f:h(diaryDay),0,6) != fn:substring(f:h(today),0,6)}">
			<span class="nextMonth">
				<s:link href="/pc/diary/nextMonth/${f:h(userInfoDto.visitMemberId)}">&gt;</s:link>
			</span>
		</c:if>
...
<c:if test="${existDiary}">
	...
	<div class="contents">
		...
		<c:forEach var="fmonth" items="${oldCal}" varStatus="status">
			...
			<dd <c:if test="${f:h(fmonth.key) eq fn:substring(f:h(userInfoDto.strEntdate),0,4)}"> class="last"</c:if>>
			<ul class="clearfix">
			...
			<c:forEach var="i" begin="0" end="11">
				...
				<c:choose>
					<c:when test="${fn:substring(f:h(today),0,6) < fn:substring(f:h(fmonth.value[i]),0,6) || fn:substring(f:h(userInfoDto.strEntdate),0,6) > fn:substring(f:h(fmonth.value[i]),0,6)}">
						<fmt:formatDate value="${f:date(fmonth.value[i],'yyyyMMdd')}" pattern="MM月" />
					</c:when>
					<c:otherwise>
						<s:link href="/pc/diary/searchMonth/${f:h(fmonth.value[i])}/${f:h(userInfoDto.visitMemberId)}">
							<fmt:formatDate value="${f:date(fmonth.value[i],'yyyyMMdd')}" pattern="MM月" />
						</s:link>
					</c:otherwise>
				</c:choose>

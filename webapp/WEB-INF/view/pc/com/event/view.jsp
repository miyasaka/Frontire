...
<title>[frontier]${f:h(communityDto.comnm)} イベント</title>
		...
		<div class="mainCommunity">
			...
			<div class="mainEvent-main" style="margin-bottom:-10px;">
				...
				<div class="listBoxCommunityEventList clearfix">
					<div class="listBoxCommunityEventListTitle">
						<table class="list_ttl">
							<tr>
								<td class="ttl_name_sp"><div class="ttl_name comColStick">イベント</div></td>
							</tr>
						</table>
					</div>
				...
				<div class="eventHead clearfix">
					<ul>
						<li>${f:h(communityDto.comnm)}&nbsp;&nbsp;イベント</li>
						<c:if test="${communityDto.makabletopic=='1' || communityDto.makabletopic=='2'}">
						<li class="pgArea">
							<div>
								<s:link href="/pc/com/entevent/${f:h(communityDto.cid)}">イベントを作成する</s:link>
							</div>
						</li>
						</c:if>
					</ul>
					...
					<div class="listBoxHead clearfix">
						<div class="ttlHeadAreaEventList">
							...
							<div class="ttlHeadAreaEventListSub clearfix">
								<ul>
									<li class="txtArea">${f:h(result[0].title)}<c:if test="${result[0].editflg=='1'}">&nbsp;&nbsp;&nbsp;&nbsp;<s:link href="/pc/com/entevent/edit/${f:h(communityDto.cid)}/${f:h(bbsid)}">編集する</s:link></c:if></li>
						...
						<div class="ttlHeadAreaEventListIntroSub clearfix">
							<ul>
								<li>
									開催日時&nbsp;：&nbsp;${f:h(result[0].eventYear)}年${f:h(result[0].eventMonth)}月${f:h(result[0].eventDay)}日<c:if test="${f:h(result[0].eventNote != '')}">（${f:h(result[0].eventNote)}）</c:if>&nbsp;&nbsp;
									開催場所&nbsp;：&nbsp;${f:h(result[0].eventareaNote)}
								</li>
							</ul>
						</div>
						...
						<div class="ttlHeadAreaEventListIntroSub clearfix" style="border-width:1px 0 0 0;">
							<ul>
								<li>
									募集期限&nbsp;：&nbsp;<c:choose><c:when test="${f:h(result[0].limitYear)!='' && f:h(result[0].limitMonth)!='' &&  f:h(result[0].limitDay)!=''}">${f:h(result[0].limitYear)}年${f:h(result[0].limitMonth)}月${f:h(result[0].limitDay)}日</c:when><c:otherwise>指定なし</c:otherwise></c:choose>
								</li>
								<li>
									参加者&nbsp;：&nbsp;${f:h(result[0].cntentry)}名<s:link href="mlist/${communityDto.cid}/${bbsid}">参加者一覧へ</s:link>
								</li>
							</ul>
						</div>
						<!-- イベントを立てた人のみ -->
						<c:if test="${f:h(result[0].eventmaker)=='1'}">
						<div class="ttlHeadAreaEventListIntroSub clearfix" style="border-width:1px 0 0 0;">
							<ul>
								<li style="text-align:right;">
									<s:link href="manage/${communityDto.cid}/${bbsid}">参加者管理画面を見る</s:link>
								</li>
							</ul>
						</div>
						</c:if>
				...
				<div class="listBoxCommunityEventList clearfix">
					...
					<div class="DelComArea">
						<c:choose>
						<c:when test="${f:h(btnlevel)=='1' || f:h(btnlevel)=='3' || f:h(btnlevel)=='5'}">
							<div style="padding:20px;">
								<input type="submit" name="finish" value="コメントのみ入力" />
							</div>
						</c:when>
						<c:when test="${f:h(btnlevel=='2')}">
							<div class="delMess clearfix">
							<ul>
							<li class="alerttxt">イベントに参加する際は参加する意思をコメント欄に記入し、参加ボタンを押してください。</li>
							</ul>
							</div>
							<div style="padding:20px;">
							<input type="submit" name="eventjoin" value="イベントに参加する" />
							<input type="submit" name="finish" value="コメントのみ入力" />
							</div>
						</c:when>
						<c:when test="${f:h(btnlevel=='4')}">
							<div class="delMess clearfix">
							<ul>
							<li class="alerttxt">イベントをキャンセルしたい場合は理由をコメント欄に記入し、キャンセルボタンを押してください。</li>
							</ul>
							</div>
							<div style="padding:20px;">
							<input type="submit" name="eventcancel" value="参加をキャンセルする" />
							<input type="submit" name="finish" value="コメントのみ入力" />
							</div>
						</c:when>
						</c:choose>

...
select
	...
	a.diaryid,
	a.comcnt as count,
	a.oya_title as l_title,
	case
		when length(a.oya_title) > 20 then substr(a.oya_title,0,21) || '・・・'
		else a.oya_title
	end as title,
	case
		when a.yd < to_char(a.max_entdate,'yyyymmdd') then to_char(a.max_entdate,'hh24:mi')
		else to_char(a.max_entdate,'mm月dd日')
	end as entdate,
	case
		when a.yd < to_char(a.oya_entdate,'yyyymmdd') then to_char(a.oya_entdate,'hh24:mi')
		else to_char(a.oya_entdate,'mm月dd日')
	end as entdate2,
	case
		when a.yd < to_char(a.max_entdate,'yyyymmdd') then to_char(a.max_entdate,'hh24:mi')
		else to_char(a.max_entdate,'mm/dd')
	end as entdatesla,
	case
		when a.yd < to_char(a.oya_entdate,'yyyymmdd') then to_char(a.oya_entdate,'hh24:mi')
		else to_char(a.oya_entdate,'mm/dd')
	end as entdate2sla
from
	(
		select
		...
		where
			 mm.status                     = '1'
			and mm.mid                     != /*mid*/
			and d2.entdate                 > (current_date - /*limitdate*/)
			and mm.mid                     = d2.mid
			and d2.delflg                  = '0'
